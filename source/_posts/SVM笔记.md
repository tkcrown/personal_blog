...
现在我们虽然可以根据这个标准求得最优超平面，但是因为$\{a w, a b| \forall a \neq 0\}$定义的是**同一个**超平面，我们需要加条件来固定$w$和$b$，以从无数$w,b$的组合中挑一个。这里我们可以选取$||w|| = c$或者$$d_{min}||w|| =c$$。取第二种的时候我们可以将$d_{min}$这个量从优化问题中消灭掉。当取$c=1$时，我们有
...
所以$g(a)$总是$p(w, b)$的下界，这就是**Weak Duality**。同时$g(a)$还一定Concave的，这个性质甚至在原目标函数和不等式约束条件非Convex的时候都成立。而我们的问题是Convex的，这（通常）都保证了**Strong Duality**，也就是说在feasible区域中，$$max_{a} g(a) = min_{w, b} p(w, b)$$。之所以说是“通常”是因为还要满足一个Slater Condition，一个非常容易满足的条件。
...
然而，给新数据点分类的过程中$$\hat{y} = w^{*T}x + b^*$$还是需要求出$$b^*$$的。其求$$b^*$$的过程非常直接，因为，假设$x_{sv}$是Support Vector，那么就有
...
1. MIT Open Course: $$b^* = 1 - min_{i:y_i=1} w^{*T}x_i$$
2. Andrew Ng: $$b^* = -(min_{i:y_i=1} w^{*T}x_i + max_{i:y_i=-1} w^{*T}x_i)/2$$
...
这是因为：
(1) 对于不满足约束条件的$w, b$，即$$\exists x_i, w^T x_i + b < 0$$，则$a_i$越大$$-a_i(y_i(w^Tx_i + b) - 1)$$越大，最终$$max_{a\geq0} L(w, b, a) = +\infty$$，所以$$min_{w, b} max_{a\geq0} L(w, b, a)$$显然不会挑选不满足约束条件的$w, b$
(2) 对于满足约束条件的$w,b$，即$$\forall x_i, w^T x_i + b \geq 0$$，则$$max_{a\geq0} \frac{1}{2} ||w||^2 - \sum_i a_i(y_i(w^Tx_i + b) - 1)$$中显然第二项为0（又称为Complementary slackness，是KKT条件的一部分，在此不做细说），所以$$max_{a\geq0} L(w, b, a) = \frac{1}{2}||w||^2$$，即优化问题又变回了$$min_{w, b} \frac{1}{2} ||w||^2$$

所以上面这个优化问题就是和原优化问题等价的。指的注意的是，它和Dual问题的区别仅仅是min-max的顺序。从这个角度我们也可以通俗的解释为什么$$d^*$$是$$p^*$$的下界。$$p^*$$是从一堆大个儿里挑最小的，而$d^*$是从一堆个小的里面挑个最大的。
